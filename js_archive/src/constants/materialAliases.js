...
export const SELLER_ADDRESS_ALIASES = {
  ...
  'Домодедово, Станционная ул., 26к3': 'Московская обл., Домодедово, Станционная ул., 26к3',
  'Станционная ул., 26к3': 'Московская обл., Домодедово, Станционная ул., 26к3'
};
 ...
 * @param {string} dateLabel - дата в формате "DDMMYY-HHmmss", "DDMMYY" или "DDMM-<runId>"
 * @returns {Date|null}
 */
export function parseDateLabel(dateLabel) {
  ...
  // Формат с runId: DDMM-<runId>
  const withRunIdMatch = dateLabel.match(/^(\d{2})(\d{2})-([a-z0-9]+)$/i);
  if (withRunIdMatch) {
    const [, dd, mm] = withRunIdMatch;
