...
const DEFAULT_WATERMARK_SETTINGS_PATH = path.resolve(__dirname, '..', 'data', 'js_watermark-settings.json');
...
function sanitizeLabelForFile(label = '') {
  return String(label)
    .split('')
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Команда ${command} завершилась с кодом ${code}${stderr ? ': ' + stderr : ''}`));
      }
    });
...
function resolveFeedCliBin() {
  const releaseBin = path.resolve(__dirname, '..', 'rust', 'target', 'release', 'feed-cli');
  const debugBin = path.resolve(__dirname, '..', 'rust', 'target', 'debug', 'feed-cli');
  if (fs.existsSync(releaseBin)) return releaseBin;
  if (fs.existsSync(debugBin)) return debugBin;
  throw new Error('feed-cli не найден. Соберите: cargo build -p feed-cli --manifest-path rust/Cargo.toml --release');
}

export async function generatePhotoForOldAd(avitoId, materialId, address, photosRoot, textWatermark, textOpacity, patternOpacity, watermarkSettings = { files: {} }) {
  ...
  try {
    ...
    const wmSettings = findWatermarkSettings(watermarkSettings, sourcePath || originals[0]);
    const effectiveTextWatermark = wmSettings?.textWatermark || textWatermark;
    const effectiveTextColor = wmSettings?.textColor || null;
    const settingsPatternOpacity =
      wmSettings && typeof wmSettings.patternOpacity === 'number' ? wmSettings.patternOpacity : undefined;
    const settingsTextOpacity =
      wmSettings && typeof wmSettings.textOpacity === 'number' ? wmSettings.textOpacity : undefined;
    ...
    const textOpacityValue = hasSettingsOpacity
...
async function main() {
  ...
  try {
    ...
    const watermarkSettingsPath = opts.watermarkSettings
      ? path.resolve(opts.watermarkSettings)
      : DEFAULT_WATERMARK_SETTINGS_PATH;
    const watermarkSettings = loadWatermarkSettings(watermarkSettingsPath);
    ...
    if (!shouldExecuteStep(5, opts.testStep)) {
      ...
      try {
      const feedCli = resolveFeedCliBin();
      const photosManifest = path.join(
        opts.outDir,
        `photos_run_${sanitizeLabelForFile(dateLabel)}.json`
      );
      const photoArgs = [
        '--plan', planPathForPhotos,
        '--photos-rust',
        '--photos-root', DEFAULT_PHOTOS_ROOT,
        '--date-label', dateLabel,
        '--photos-manifest', photosManifest
      ];
      console.log(`   Запуск Rust-генерации фото (feed-cli)...`);
      await runCommand(feedCli, photoArgs);
    ...
    if (!shouldExecuteStep(6, opts.testStep)) {
      ...
      try {
        ...
        if (opts.dryRun) {
          ...
          console.log(`   Загрузка фото для новых объявлений (Rust upload)...`);
          const feedCli = resolveFeedCliBin();
          const photosManifest = path.join(
            opts.outDir,
            `photos_run_${sanitizeLabelForFile(dateLabel)}.json`
          ...
          const uploadArgs = [
            ...
            '--upload-rust-only',
            '--photos-root', DEFAULT_PHOTOS_ROOT,
            '--photos-dir', DEFAULT_PHOTOS_ROOT,
            '--photos-manifest', photosManifest,
            '--disk-root', opts.diskRoot,
            '--date-label', dateLabel,
            '--out-dir', opts.outDir
          ];

          await runCommand(feedCli, uploadArgs, { env: { YANDEX_DISK_TOKEN: token } });
        }
