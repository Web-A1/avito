...
pub fn generate_new_ads(
    ...
    for slot in &plan.publication_queue {
        ...
        let counter = counters.entry(key.clone()).and_modify(|c| *c += 1).or_insert(1);
        ...
        let ad_id = generate_ad_id(&material_id, &address, &dt, *counter)
            .map_err(|e| format!("Ошибка генерации adId для {} @ {}: {:?}", material_id, address, e))?;

        if existing_ids.contains(&ad_id) {
            return Err(format!("Сгенерированный adId уже существует в Excel: {}", ad_id));
        }
