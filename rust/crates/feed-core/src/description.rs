...
fn build_block2_rubble(mat: &str, rng: &mut rand::rngs::ThreadRng) -> String {
    ...
    build_block2(
        ...
        pick(headings, rng),
        list_html,
        "Минимальный объём поставки — 20 м³ (1 самосвал)",
    )
}
...
fn build_block2(intro: String, heading: String, list_html: &str, minimum: &str) -> String {
    ...
    format!(
        "{}<p>{}</p>{}<p>{}</p>",
        intro_html, heading, list_html, minimum
    )
}
...
const BLOCK_4_ADVANTAGES_HTML: &str = "<p>Преимущества NERUDA:<br>- Напрямую из карьера: мы получаем нерудные материалы напрямую из карьера, без посредников<br>- Собственный автопарк: самосвалы различной грузоподъемности, фронтальные погрузчики, гусеничные экскаваторы, бульдозеры<br>- Порядок с документами: оформляем все официально и строго следим за качеством документооборота, что исключает спорные ситуации и гарантирует юридическую защиту для Заказчика<br>- Платите, как Вам удобно: наличные, карта, перевод, по счету</p>";
