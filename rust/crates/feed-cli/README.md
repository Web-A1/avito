...
- `--photos-text`/`--photos-text-opacity`/`--photos-text-color`/`--photos-pattern-opacity` — настройки ватермарка
- `--photos-test-dir` — тестовый каталог для фото (не пишет историю в data/photos)
...
- `--make-watermark-settings "<glob>"` — сгенерировать шаблон `data/watermark-settings.json` по маске файлов (например, `"data/photos/**/*.jpg"`)
...
Сейчас выполняет валидации плана (counts/окна/шаги). Фото-обвязка через JS есть (`--photos`), читает маппинг по `photos_links_<date_label>.json` (или указанному). Rust-генерация (`--photos-rust`) использует `data/watermark-settings.json` для каждого исходника, складывает фото в `--photos-dir` или `--photos-test-dir`. Загрузка на Я.Диск (`--upload-rust`) формирует `photos_links_<label>.json`.
