...
pub fn generate_photos(plan: &PathBuf) -> Result<(), String> {
    ...
    if !status.success() {
        return Err(format!("generate-photo-variants.js завершился с кодом {:?}", status.code()));
    }
    Ok(())
}

/// Вызывает JS-скрипт загрузки фото на Я.Диск.
pub fn upload_photos(plan: &PathBuf, disk_root: &str, date_label: &str, out_dir: &PathBuf) -> Result<(), String> {
    let status = Command::new("node")
    ...
    if !status.success() {
        return Err(format!("upload-photos.js завершился с кодом {:?}", status.code()));
    }
    Ok(())
}
