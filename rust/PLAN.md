...
1. Выписать актуальные шаблоны описаний из JS (`src/generators/materials/sand/descriptionGenerator.js`, `.../rubble/descriptionGenerator.js`, `src/constants/block1And2.js`, `src/constants/blocks.js`, `algorithms/separatorGenerator.js`, `blockOrderGenerator.js`): блоки 1–7, интро для блока 2, разделители, порядок блоков.
2. Перенести эту логику в Rust (`feed-core/src/description.rs`): либо без рандома (фиксированный вариант), либо повторить вариативность как в JS. Обновить генерацию описаний для песка/щебня, чтобы текст/разметка совпадали.
3. Перегенерировать XML и сравнить с JS-эталоном (`output/ads_22.12.xml`), зафиксировать остаточные расхождения и устранить.
